...
import { ResGetProductStatisticByOrder, ResGetProductStatisticByRevenue, ResGetProductStatisticBySoldQuantity, ResGetRevenueStatisticByCategory, ResGetRevenueStatisticByCustomer, ResGetRevenueStatisticByDate, StatisticQuery } from "@/types/statistic";


class StatisticService {
    async getOverviewStatistic(query: StatisticQuery): Promise<{ data: { data: { revenue: number; totalOrders: number } } }> {
        let url = 'statistic/get-statistic?fromDate=' + query.fromDate + '&toDate=' + query.toDate;
        if (query.status && query.status in OrderStatus) url += '&status=' + query.status;
        return api.get(url);
    }
    async getProductStatisticByRevenue(query: StatisticQuery): Promise<ResGetProductStatisticByRevenue> {
        ...
        if (query.status && query.status in OrderStatus) url += '&status=' + query.status;
        if (query.top) url += '&top=' + query.top;
        return api.get(url);
    }
    ...
    async getRevenueStatisticByDate(query: StatisticQuery): Promise<ResGetRevenueStatisticByDate> {
        const url = 'statistic/get-revenue-statistic-date?fromDate=' + query.fromDate + '&toDate=' + query.toDate;
        return api.get(url);
    }
